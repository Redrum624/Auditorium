 ...
 * then refuses, because there would be no grid. `Effects → Match Tempo…` is
...
function readExtra(): MatchTempoVariableExtra {
  ...
  if (!Array.isArray(beats) || beats.length < 2 || !Number.isFinite(targetSpacing) || (targetSpacing as number) <= 0) {
    ...
    throw new Error(
      'Variable-rate Match Tempo needs a confirmed beat grid. Open Effects → Match Tempo… rather than running this effect directly.'
    );
  }
