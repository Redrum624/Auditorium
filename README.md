...
with speaker labels that exports as SRT or WebVTT, a voice changer, and lyric
alignment that places every word of a set of lyrics against the recording so
you can replace one of them by singing it again — all processing runs
...
- **Cover Chain Dialog** — the nine stages of matching a new vocal take to the original singer's, listed in the order they run with the note saying why each sits there, the four automatic ones switchable on their own and the five manual ones (separate the original, the Vocal Chain, Align Lyrics, Align Vocal Timing, place it on the instrumental) listed in their place with the reason each is yours to run; a picker for the separated original vocal to match against; the three measured caveats stated ABOVE the button (the bed still contains the original singer, the match is a shaping and not a transformation, a single take still has to be a good take); and afterwards a per-band table of what the EQ was asked for, what it actually realised and what gain it was handed, plus a before/after table of loudness, envelope spread, noise floor and spectral distance from the original vocal against the original vocal's own column.
